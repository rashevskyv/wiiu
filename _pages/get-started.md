...
Прежде всего, нужно убедиться, что прошивка вашей приставки 5.5.0-{% include /vars/sys_version.txt %}. Более старые версии пока не поддерживаются.
...
Второй метод потребует покупки недорогой игры от DS Virtual Console, через которую будет автоматически запускаться кастомная прошивка при запуске приставки. Этот метод рекомендован для пользователей {% include /vars/sys_version.txt %} из-за нестабильности способа с браузером.
...
Для того, чтобы войти в eShop на прошивках ниже, чем {% include /vars/sys_version.txt %}, вам снова понадобится запустить Homebrew Launcher и из него запустить NNU-Patcher, и только после этого вы сможете войти в eShop и купить игру, либо, если не помогло, обновиться до {% include /vars/sys_version.txt %} через настройки приставки.
{: .notice--warning}
...
Этот метод использует недорогую игру от DS Virtual Console, через которую будет запускаться кастомная прошивка при запуске приставки. Рекомендуется использовать этот метод, несмотря на то, что он рребует небольших денежных вложений. Mocha CFW заброшена и давно не развивается, используйте её только, если вы очень жадный. *Рекомендуемый* метод для пользователей приставок с прошивкой {% include /vars/sys_version.txt %}, поскольку веб-эксплойт из предыдущего метода крайне нестабилен. Так же только используя Рфчсрш вы сможете сделать запуск эксплойта автоматически, с помощью CBHC.
