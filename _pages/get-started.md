...
Первый метод подразумевает запуск через браузер приставки эксплойта, который патчит систему. К сожалению, это придется делать после каждого перезапуска приставки. 

Второй метод потребует покупки недорогой игры от DS Virtual Console, через которую будет автоматически запускаться кастомная прошивка при запуске приставки. Этот метод рекомендован для большинства пользователей.

Игра для DS Virtual Console должна быть **законно приобретённой** и установленной во **внутреннюю память** приставки. **Не** устанавливайте игру от DS virtual console на USB-носитель. В данный момент купить игру напрямую с карты у вас не выйдет. Для того, чтобы купить игру в eShop, нужно использовать gift-карты. Минимальная стоимость такой карты 10$ или 750 рублей. [Подробнее](https://www.nintendo.ru/-/Nintendo-2DS-Nintendo-3DS/-/Nintendo-eShop/-Nintendo-eShop--1626660.html){: target="_blank"}
{: .notice--warning}
